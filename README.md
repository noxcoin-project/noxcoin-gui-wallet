...
- Github: [https://github.com/noxcoin-project/noxcoin-gui](https://github.com/noxcoin-project/noxcoin-gui)
- IRC: [#noxcoin-gui on Libera](irc://irc.libera.chat/#noxcoin-gui)
- Translation platform (Weblate): [translate.noxcoin.org](https://translate.noxcoin.org)
- UI Design: [Noxcoin-GUI on Figma](https://www.figma.com/file/DplJ2DDQfIKiuRvolHX2hN/Noxcoin-GUI)
...
This is the GUI for the [core Noxcoin implementation](https://github.com/noxcoin-project/noxcoin). It is open source and completely free to use without restrictions, except for those specified in the license agreement below. There are no restrictions on anyone creating an alternative implementation of Noxcoin that uses the protocol and network in a compatible manner.
...
Noxcoin is a 100% community-sponsored endeavor. If you want to join our efforts, the easiest thing you can do is support the project financially. Both Noxcoin and Bitcoin donations can be made to **donate.noxcoin.org** if using a client that supports the [OpenAlias](https://openalias.org) standard.

The Noxcoin donation address is: `888tNkZrPN6JsEgekjMnABU4TBzc2Dt29EPAvkRxbANsAnjyPbb3iQ1YBRk1UXcdRsiKc9dhwMVgN5S9cQUiyoogDavup3H` (viewkey: `f359631075708155cc3d92a32b75a7d02a5dcf27756707b47a2b31b21c389501`)

The Bitcoin donation address is: `1KTexdemPdxSBcG55heUuTjDRYqbC5ZL8H`

GUI development funding and/or some supporting services are also graciously provided by [sponsors](https://www.noxcoin.org/community/sponsorships/):
...
Do you speak a second language and would like to help translate the Noxcoin GUI? Check out Weblate, our localization platform, at [translate.noxcoin.org](https://translate.noxcoin.org/). Choose the language and suggest a translation for a string or review an existing one. The Localization Workgroup made [a guide with step-by-step instructions](https://github.com/noxcoin-ecosystem/noxcoin-translations/blob/master/weblate.md) for Weblate.

If you need help/support or any info you can contact the localization workgroup on the IRC channel #noxcoin-translations (relayed on [Matrix](https://matrix.to/#/!BKMbQLMDzHKzmtrBfg:matrix.org?via=noxcoin.social&via=matrix.org&via=libera.chat)) or by email at translate[at]getnoxcoin[dot]org. For more info about the Localization workgroup: [github.com/noxcoin-ecosystem/noxcoin-translations](https://github.com/noxcoin-ecosystem/noxcoin-translations)

Status of the translations:  

<a href="https://translate.noxcoin.org/engage/noxcoin/?utm_source=widget">
<img src="https://translate.noxcoin.org/widgets/noxcoin/-/gui-wallet/horizontal-auto.svg" alt="Translation status" />
</a>

## Installing the Noxcoin GUI from a package

Packages are available for
* Arch Linux: [noxcoin-gui](https://archlinux.org/packages/extra/x86_64/noxcoin-gui/)
* NixOS: `nix-shell -p noxcoin-gui`
* Flatpak: [Noxcoin GUI](https://flathub.org/apps/details/org.getnoxcoin.Noxcoin)
* GuixSD: `guix package -i noxcoin-gui`
* macOS (homebrew): `brew install --cask noxcoin-wallet`
...
*Note*: Official GUI releases use noxcoin-wallet-gui from this process alongside the supporting binaries (noxcoind, etc) from the [CLI deterministic builds](https://github.com/noxcoin-project/noxcoin/blob/release-v0.18/contrib/gitian/README.md).
...
4. Build
   ```
   docker run --rm -it -v <MONERO_GUI_DIR_FULL_PATH>:/noxcoin-gui -w /noxcoin-gui noxcoin:build-env-windows sh -c 'make depends root=/depends target=x86_64-w64-mingw32 tag=win-x64 -j4'
   ```
   \* `<MONERO_GUI_DIR_FULL_PATH>` - absolute path to `noxcoin-gui` directory  
   \* `4` - number of CPU threads to use
...
4. Build
   ```
   docker run --rm -it -v <MONERO_GUI_DIR_FULL_PATH>:/noxcoin-gui -w /noxcoin-gui noxcoin:build-env-linux sh -c 'make release-static -j4'
   ```
   \* `<MONERO_GUI_DIR_FULL_PATH>` - absolute path to `noxcoin-gui` directory  
   \* `4` - number of CPU threads to use
5. Noxcoin GUI Linux static binary will be placed in  `noxcoin-gui/build/release/bin` directory
6. (*Note*) This process is only for building `noxcoin-wallet-gui`, `noxcoind` has to be built separately according to the instructions in the `noxcoin` repository.
7. (*Optional*) Compare `noxcoin-wallet-gui` SHA-256 hash to the one obtained from a trusted source
   ```
   docker run --rm -it -v <MONERO_GUI_DIR_FULL_PATH>:/noxcoin-gui -w /noxcoin-gui noxcoin:build-env-linux sh -c 'shasum -a 256 /noxcoin-gui/build/release/bin/noxcoin-wallet-gui'
   ```
   \* `<MONERO_GUI_DIR_FULL_PATH>` - absolute path to `noxcoin-gui` directory  
...
4. Build
   ```
   docker run --rm -it -v <MONERO_GUI_DIR_FULL_PATH>:/noxcoin-gui -e THREADS=4 noxcoin:build-env-android
   ```
   \* `<MONERO_GUI_DIR_FULL_PATH>` - absolute path to `noxcoin-gui` directory  
   \* `4` - number of CPU threads to use
...
3. Install [noxcoin](https://github.com/noxcoin-project/noxcoin) dependencies:
...
3. Install MSYS2 packages for Noxcoin dependencies; the needed 64-bit packages have `x86_64` in their names
    ...
    You find more details about those dependencies in the [Noxcoin documentation](https://github.com/noxcoin-project/noxcoin). Note that that there is no more need to compile Boost from source; like everything else, you can install it now with a MSYS2 package.
